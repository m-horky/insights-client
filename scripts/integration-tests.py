...
def setup() -> None:
    ...
    logger.debug(f"Ensuring {MISC_DIR!s} exists")
...
def cleanup() -> None:
    ...
    logger.debug(f"Removing {MISC_DIR!s}")
...
def build_image() -> None:
    ...
    if proc.returncode == 0:
        logging.debug(f"`podman build` returned code {proc.returncode}")
    else:
        logging.error(f"`podman build` returned code {proc.returncode}")
    logging.info(f"You can now run tests: `run-tests --settings [FILE]`")
...
def _start_container(*, settings: pathlib.Path, egg: Optional[pathlib.Path]) -> str:
    ...
    egg_config = (
        []
        if egg is None
        else ["-v", f"{egg.absolute()!s}:/egg.zip:ro", "-e", "EGG=/egg.zip"]
    )
    start_command = [
        *start_exec,
        *hostname,
    ...
    logger.info(" ".join(start_command))
    ...
    logger.debug("Waiting for systemd")
    time.sleep(5)
...
def _remove_container(container: str) -> None:
    logger.debug(f"Tearing down container {container}")

    shutdown_exec = ["podman", "container", "rm", "-f"]
    shutdown_command = [*shutdown_exec, container]

    logger.info(" ".join(shutdown_command))
...
def run_tests(*, settings: pathlib.Path, egg: Optional[pathlib.Path]) -> None:
    ...
    logger.debug("Running the tests")
...
def run_shell(*, settings: pathlib.Path, egg: Optional[pathlib.Path] = None) -> None:
    ...
    logger.info(" ".join(run_command))
    logger.info("You can start the test manually: `bash test.sh`")
    run_proc = subprocess.Popen(run_command, text=True)
    run_proc.communicate()
...
def main():
    ...
    parser_run_tests = subparser.add_parser("run-tests", help=f"run integration tests")
